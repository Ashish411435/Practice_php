...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($password != $confpassword) {
            ...
            if ($row_count > 0) {
                $_SESSION['alreadyAlert'] = "Email Already Exist...";
                ...
                $result = mysqli_query($con, $sql_query);
                if ($result) {
                    $_SESSION['successInsert'] = "Successfully Registered";
                    header("Location: admin_register.php");
                    exit();
                } else {
                    $_SESSION['failedInsert'] = "Oops Something Went Wrong...";
                    echo $con->error;
                }
            }
        }
    ...
    <?php
    if (isset($_SESSION['alreadyAlert'])) {
        echo '<div class="alert alert-danger" role="alert">' . $_SESSION['alreadyAlert'] . '</div>';
        unset($_SESSION['alreadyAlert']);
    }
    if (isset($_SESSION['passNotMatch'])) {
        echo '<div class="alert alert-warning" role="alert">' . $_SESSION['passNotMatch'] . '</div>';
    ...
    if (isset($_SESSION['failedInsert'])) {
        echo '<div class="alert alert-danger" role="alert">' . $_SESSION['failedInsert'] . '</div>';
        unset($_SESSION['failedInsert']);
    }
    ?>
